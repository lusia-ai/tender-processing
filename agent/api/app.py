...
from pdf_agent.utils.io.checkpoint import build_checkpointer
...
def _settings() -> Dict[str, Any]:
    ...
    return {
        ...
        "embed_model": require_env("OPENAI_EMBED_MODEL"),
        "dimensions": int(require_env("OPENAI_EMBED_DIM")),
...
@lru_cache(maxsize=1)
def _checkpointer() -> Optional[object]:
    from os import getenv

    enabled = getenv("ENABLE_CHECKPOINTING", "true").lower() in {"1", "true", "yes"}
    if not enabled:
        return None
    ttl_seconds = int(getenv("CHECKPOINT_TTL_SECONDS", "0"))
    max_threads = int(getenv("CHECKPOINT_MAX_THREADS", "0"))
    return build_checkpointer(max_threads=max_threads, ttl_seconds=ttl_seconds)
