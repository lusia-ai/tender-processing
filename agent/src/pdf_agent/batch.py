...
from pdf_agent.chunker import Chunk, iter_chunks

DEFAULT_INPUT_DIR = Path(__file__).resolve().parent.parent / "data" / "tenders" / "raw"
DEFAULT_OUTPUT = Path(__file__).resolve().parent.parent / "data" / "tenders" / "processed" / "chunks.jsonl"
...
def _build_args() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=str,
        default="gpt-4o-mini",
        help="LLM model for semantic-llm chunking.",
    )
    parser.add_argument(
        "--api-key",
        type=str,
