...
def _build_args() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Optional .env file to load (defaults to src/.env).",
    )
    parser.add_argument(
        "--chunks-file",
        type=str,
        default=str(Path(__file__).resolve().parent.parent / "data" / "tenders" / "processed" / "chunks.jsonl"),
        help="Path to JSONL produced by pdf_agent.batch",
    )
    parser.add_argument(
        "--output",
        type=str,
        default=str(Path(__file__).resolve().parent.parent / "data" / "tenders" / "processed" / "embeddings.jsonl"),
    ...
    parser.add_argument(
        ...
        help="Model name (OpenAI or sentence-transformers model id)",
    )
    ...
    parser.add_argument(
        "--dimensions",
        type=int,
        help="Override OpenAI embedding dimensions (e.g. 1024 to match a Pinecone index). OpenAI default: 3072 for text-embedding-3-large.",
    )
