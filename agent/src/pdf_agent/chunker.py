...
from pdf_agent.parser import PageText, extract_pages_text
...
def _llm_is_new_topic(
    ...
    system = (
        "You detect topic shifts between adjacent text blocks. "
        "Return ONLY 'YES' if the second block starts a new topic or section, otherwise 'NO'. "
        "Treat headings as part of the next block. Do not explain."
    )
    user = (
        "Block A:\n"
        f"{prev_block}\n\n"
        "Block B:\n"
        f"{next_block}\n\n"
        "Does Block B start a new topic? Answer YES or NO only."
    )
    if log_fn:
