...
from pdf_agent.parsing.parser import extract_pages_text
...
def _build_args() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--input-dir",
        type=str,
        default=str(Path(__file__).resolve().parents[2] / "data" / "tenders" / "raw"),
    )
    parser.add_argument(
        "--output",
        type=str,
        default=str(Path(__file__).resolve().parents[2] / "data" / "tenders" / "processed" / "lots.jsonl"),
    )
